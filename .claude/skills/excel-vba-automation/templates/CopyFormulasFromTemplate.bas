Attribute VB_Name = "CopyFormulasFromTemplate"
Option Explicit

'==============================================================================
' ひな型Excelから関数（数式）コピーテンプレート
'------------------------------------------------------------------------------
' 概要   : 実行時に選択した「ひな型Excel」の指定シート・指定範囲の数式
'          （関数）を、フォルダ内の全Excelファイルの同じ位置へコピーします。
'          書式は変更しません。
' 使い方 : VBE（Alt+F11）→ 挿入 → 標準モジュール に全文を貼り付けて、
'          CopyFormulasToAllFiles を実行してください。
...
Public Sub CopyFormulasToAllFiles()
    ...
    Dim rangeAddress As String      ' 対象範囲（例: G2:G100）
    ...
    targetFolder = SelectFolder("数式を反映するExcelファイルがあるフォルダを選択してください")
    If targetFolder = "" Then Exit Sub

    '--- 3) 対象シート名・対象範囲をユーザーに確認する ---
    sheetName = InputBox("対象のシート名を入力してください", "シート名の確認", "Sheet1")
    If sheetName = "" Then Exit Sub
    rangeAddress = InputBox("数式をコピーする範囲を入力してください（例: G2:G100）", "範囲の確認", "G2:G100")
    ...
    Do While fileName <> ""
        ...
        Else
            ...
            If Err.Number = 0 Then
                CopyFormulasToWorkbook wbTemplate, wb, sheetName, rangeAddress
            End If
    ...
    '--- 8) ログファイルを出力する ---
    logPath = targetFolder & "\数式コピーログ_" & Format(Now, "yyyymmdd_hhnnss") & ".txt"
...
'------------------------------------------------------------------------------
' ひな型の数式を1ブックへコピーする（書式は変更しない）
'------------------------------------------------------------------------------
Private Sub CopyFormulasToWorkbook(ByVal wbTemplate As Workbook, ByVal wb As Workbook, _
    ...
    ' 数式のみを貼り付ける（xlPasteFormulas = 数式だけ、相対参照はセル位置に合わせてずれます）
    wsFrom.Range(rangeAddress).Copy
    wsTo.Range(rangeAddress).PasteSpecial Paste:=xlPasteFormulas
